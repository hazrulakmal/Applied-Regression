...
ks4score.all.lm <-lm(ks4score ~ factor(fiveac)+factor(fiveem)
                     ...
                     +factor(gender)+factor(SECshort)+factor(hiquamum)
                     +factor(singlepar)+factor(house)+factor(fsm)
                     +factor(parasp)+factor(computer)+factor(tuition)+factor(pupasp)
                     +factor(homework)+factor(attitude)+factor(sen)+factor(truancy)
                     +factor(absent)+factor(exclude)+IDACI_n+factor(FSMband)
                     ,data=rw.data)
Anova(ks4score.all.lm)

##---remove SECshort and fsm which has the largest p-value

ks4score.all.lm2 <-lm(ks4score ~ factor(fiveac)+factor(fiveem)
                     +factor(k3en)+factor(k3ma)+factor(k3sc)
                     +factor(gender)+factor(hiquamum)
...
##---remove hiquamum which has the largest p-value
...
ks4score.all.lm5 <-lm(ks4score ~ factor(fiveac)+factor(fiveem)
                      ...
                      +factor(parasp)+factor(computer)+factor(pupasp)
                      +factor(homework)+factor(attitude)+factor(sen)+factor(truancy)
                      +factor(absent)+factor(exclude)+factor(FSMband)
                      ,data=rw.data)
...
##---compare the boxplots carefully the value of exclude vary quite a lot with truancy and absent which have quite similar values


##---remove the missing values of the remaining predictors

new.rw.data <- subset(rw.data, singlepar!="missing"&house!="other/DK/Ref"&computer!="missing"&
...
ks4score.k3ma <-lm(ks4score ~ factor(k3ma) ,data=new.rw.data)
summary(ks4score.k3ma)
##---Multiple R-Squared = 0.4842

ks4score.k3ma.truancy <-lm(ks4score ~ factor(k3ma)+factor(truancy),data=new.rw.data)
summary(ks4score.k3ma.truancy)
##---Multiple R-Squared = 0.4942

ks4score.k3ma.absent <-lm(ks4score ~ factor(k3ma)+factor(absent),data=new.rw.data)
summary(ks4score.k3ma.absent)
##---Multiple R-Squared = 0.4856

ks4score.k3ma.exclude <-lm(ks4score ~ factor(k3ma)+factor(exclude),data=new.rw.data)
...
##---fiveav and fiveem
ks4score.k3ma.fiveac <-lm(ks4score ~ factor(k3ma)+factor(fiveac),data=new.rw.data)
summary(ks4score.k3ma.fiveac)
##---Multiple R-Squared = 0.6471

ks4score.k3ma.fiveem <-lm(ks4score ~ factor(k3ma)+factor(fiveem),data=new.rw.data)
...
p26<-ggplot(new.rw.data, aes(x=pupasp, y=ks4score)) + geom_boxplot()
grid.arrange(p25, p26, ncol =2)

ks4score.k3ma.newhom <-lm(ks4score ~ factor(k3ma)+factor(new_homework),data=new.rw.data)
summary(ks4score.k3ma.newhom)
##---Multiple R-Squared = 0.4957

ks4score.k3ma.pupasp <-lm(ks4score ~ factor(k3ma)+factor(pupasp),data=new.rw.data)
...
ks4score.k3ma.newatt <-lm(ks4score ~ factor(k3ma)+factor(new_attitude),data=new.rw.data)
summary(ks4score.k3ma.newatt)
##---Multiple R-Squared = 0.4949



##---singlepar, house and computer
grid.arrange(p9, p10, p13, ncol =2, nrow=2)

ks4score.k3ma.sgp <-lm(ks4score ~ k3ma+factor(singlepar),data=new.rw.data)
summary(ks4score.k3ma.sgp)
##---Multiple R-Squared = 0.4899

ks4score.k3ma.house <-lm(ks4score ~ k3ma+factor(house),data=new.rw.data)
summary(ks4score.k3ma.house)
##---Multiple R-Squared = 0.4909

ks4score.k3ma.cpt <-lm(ks4score ~ k3ma +factor(computer),data=new.rw.data)
...
ks4score.all.lm7 <-lm(ks4score ~ factor(fiveac)
                      +factor(new_meank3s)
                      +factor(gender)
                      +factor(house)
                      +factor(pupasp)
...
summary(ks4score.all.lm7)
##---as new_FSMband is non-significant at 5% level, remove it.

finalModel <-lm(ks4score ~ factor(fiveac)
                +factor(new_meank3s)
                +factor(gender)
                +factor(house)
                +factor(pupasp)
...
hist(finalModel$residuals,main="Histogram of residuals",front.main=1,xlab="residuals")
