...
p3 <- ggplot(rw.data, aes(x= k3en, y=ks4score)) + geom_boxplot() + labs(x="English Score Tier")
p4 <- ggplot(rw.data, aes(x= k3ma, y=ks4score)) + geom_boxplot() + labs(x="Maths Score Tier")
p5 <- ggplot(rw.data, aes(x= k3sc, y=ks4score)) + geom_boxplot() +  labs(x="Science Score Tier")
...
hist(ks4score.all.lm$residuals,main="Histogram of residuals",xlab="residuals")
...
par(mfrow=c(2,2))
plot(ks4score.all.lm5,which = c(1,2))
hist(ks4score.all.lm5$residuals,main="Histogram of residuals",xlab="residuals")
...
new.rw.data.2 <- subset(new.rw.data, singlepar!="missing"&house!="other/DK/Ref"&computer!="missing"&
                          homework!="missing"&new_attitude!="missing"&sen!="missing"&
                          truancy!="missing"&absent!="missing"&exclude!="missing"&
                          new_FSMband!="NA"&new_hiquamum!="missing"&is.na(IDACI_n)==FALSE)
...
par(mfrow=c(2,2))
plot(ks4score.all.lm6,which = c(1,2))
hist(ks4score.all.lm6$residuals,main="Histogram of residuals",xlab="residuals")
...
vif(ks4score.all.lm8)
##---All remaining predictors are statistical significant at 5% level. 

par(mfrow=c(2,2))
plot(ks4score.all.lm8,which = c(1,2))
hist(ks4score.all.lm8$residuals,main="Histogram of residuals",xlab="residuals")


##--k3ma, k3en, k3sc
ks4score.lmk3en <-lm(ks4score ~ factor(fiveac)+factor(fiveem)
                   + k3en
                   +factor(gender)+factor(new_hiquamum)
                   +factor(singlepar)+factor(house)
                   +factor(computer)+factor(pupasp)
                   ...
                   +factor(exclude)+factor(new_FSMband)
                   ,data=new.rw.data.2)
summary(ks4score.lmk3en)
##---Multiple R-Squared =  0.6552 RRE = 82.1 

ks4score.lmk3ma <-lm(ks4score ~ factor(fiveac)+factor(fiveem)
                   + k3ma 
                   +factor(gender)+factor(new_hiquamum)
                   +factor(singlepar)+factor(house)
                   +factor(computer)+factor(pupasp)
                   ...
                   +factor(exclude)+factor(new_FSMband)
                   ,data=new.rw.data.2)
summary(ks4score.lmk3ma)
##---Multiple R-Squared =  0.6638 RRE = 81.08

ks4score.lmk3sc <-lm(ks4score ~ factor(fiveac)+factor(fiveem)
                   + k3sc
                   +factor(gender)+factor(new_hiquamum)
                   +factor(singlepar)+factor(house)
                   +factor(computer)+factor(pupasp)
                   ...
                   +factor(exclude)+factor(new_FSMband)
                   ,data=new.rw.data.2)
summary(ks4score.lmk3sc)
##---Multiple R-Squared =  0.6606 RRE = 81.46 
...
ks4score.k3ma <-lm(ks4score ~ k3ma ,data=new.rw.data.2)
summary(ks4score.k3ma)
##---Multiple R-Squared = 0.4839

ks4score.k3ma.fiveac <-lm(ks4score ~ k3ma+factor(fiveac),data=new.rw.data.2)
summary(ks4score.k3ma.fiveac)
##---Multiple R-Squared = 0.6349

ks4score.k3ma.fiveem <-lm(ks4score ~ k3ma+factor(fiveem),data=new.rw.data.2)
summary(ks4score.k3ma.fiveem)
##---Multiple R-Squared = 0.5565
##---as fiveac has the higher Multiple R-Squared value, omit fiveem
##--- both predictors are highly correlated (vif)

ks4score.all.lm10 <-lm(ks4score ~ factor(fiveac)
                       + k3ma
                       +factor(gender)+factor(new_hiquamum)
                       +factor(singlepar)+factor(house)
                       +factor(computer)+factor(pupasp)
...
ks4score.k3ma.truancy <-lm(ks4score ~ k3ma+factor(truancy),data=new.rw.data.2)
summary(ks4score.k3ma.truancy)
##---Multiple R-Squared = 0.4943

ks4score.k3ma.exclude <-lm(ks4score ~ k3ma+factor(exclude),data=new.rw.data.2)
summary(ks4score.k3ma.exclude)
##---Multiple R-Squared = 0.504
##---as exclude has the highest Multiple R-Squared value, omit truancy


##---new_homework and pupasp
ks4score.k3ma.newhom <-lm(ks4score ~ k3ma+factor(new_homework),data=new.rw.data.2)
summary(ks4score.k3ma.newhom)
##---Multiple R-Squared = 0.4958

ks4score.k3ma.pupasp <-lm(ks4score ~ k3ma+factor(pupasp),data=new.rw.data.2)
summary(ks4score.k3ma.pupasp)
##---Multiple R-Squared = 0.4984
##---as pupasp has the higher Multiple R-Squared value, omit new_homework
...
summary(ks4score.all.lm11)

par(mfrow=c(2,2))
plot(ks4score.all.lm11,which = c(1,2))
hist(ks4score.all.lm11$residuals,main="Histogram of residuals",xlab="residuals")
...
p30 <- ggplot(new.rw.data.2, aes(x = factor(fiveac),y = ks4score, fill = factor(gender))) + geom_bar(position = "dodge", stat = "identity") 
p30 <- p30 + theme_bw() + scale_fill_grey()
p30
